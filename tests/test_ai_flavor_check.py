...
def test_report_output_dir_relativized(tmp_path, monkeypatch):
    monkeypatch.setattr(AFC, "ROOT", str(tmp_path))
    out = tmp_path / "小红书"
    out.mkdir()
    (out / "文案.md").write_text("这是一条正常内容，没有 AI 腔。", encoding="utf-8")
    r = AFC.run(str(out))
    assert r["output_dir"] == "小红书"
    assert not r["output_dir"].startswith("/")
