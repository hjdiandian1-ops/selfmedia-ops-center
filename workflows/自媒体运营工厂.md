...
- **发布就绪（草稿箱模式）**：呈现最终 Checklist 与成果文件卡片，等待用户回复“确认发布”后调用 [publish_to_n8n.py](file:///Users/xiaowuliao/Projects/自媒体发布agent/scripts/publish_to_n8n.py) 发送至微信公众号草稿箱和小红书本地素材准备包，由用户在手机端进行最后手动发送外网。
  - **主链路（P1 起）**：`publish_to_n8n.py` 默认直连 NAS `xhs_publisher`（5800 端口）；n8n Webhook 仅为降级链路。
  - **公众号草稿**：`publish_to_n8n.py --draft --gzh-html outputs/<job_id>/公众号/<排版.html>` 把排版 HTML 存入公众号草稿箱，人工手机终审。
  - **公众号草稿（官方 API，推荐）**：`python3 scripts/gzh_draft_api.py --title ... --content-file ... --cover ... --job-id <job_id>` 调用微信 `draft/add` 接口，绕开浏览器编辑器，稳定可靠。
  - 发布成功后自动落盘 `jobs/<job_id>/publish_log.json`。
