...
* **小红书主编 (Xiaohongshu Chief Editor Agent)**：
  ...
  - **数据可视化（硬指标）**：关键数字对比必须落在卡片条形/占比组件上（`h-bar-chart`/`bar-tower`，C12 校验），禁止纯文字罗列数字。
...
* **公众号主编 (WeChat Longform Chief Editor Agent)**：
  ...
  - **必须按 [产出标准.md](file:///Users/xiaowuliao/Projects/自媒体发布agent/workflows/产出标准.md) 的「深度长文结构模板（硬核拆解型）」写作**：Hook → 账本 → 渗透率 → 机制解剖 → 稀缺品论证 → 行动判断 → 升华收尾；**禁止独立 NAS/自我实证章节**，个人工作流只作一句话案例。
  - **真实感清单（硬指标）**：≥1 段第一人称经历/观察（来自种子素材；无种子素材时向用户索取，严禁脑补虚构）。
  - **数据可视化（硬指标）**：正文必须包含 ≥2 个 `data-viz` 组件（表格/条形/占比/KPI），复杂对比用 PNG 图卡；统一调用 `scripts/generate_data_viz.py`，数据只取素材包 `真实数据/用户投喂`（规范见 `workflows/数据可视化规范.md`）。
...
* **美术总监 (Visual Design Director Agent)**：
  ...
  - AI 封面/配图先调用 [cover-design-skill](file:///Users/xiaowuliao/Projects/自媒体发布agent/skills/cover-design-skill/SKILL.md) 走 8 问构图确认（10 风格、图1 人脸参考、红白品牌色），再调用 [generate_ai_image.py](file:///Users/xiaowuliao/Projects/自媒体发布agent/scripts/generate_ai_image.py) 生成，并过 `check_cover_specs.py` 规格检查。
...
* **资深校对排版 (Chief Reviewer & Layout Editor Agent)**：
  ...
  - **P0 新增硬门（C8-C10）**：`真实数据` 无来源链接 / 出现「链接待补」→ FAIL；小红书 <5 标签或无互动引导 → FAIL；公众号整段重复、参考来源无链接 → FAIL；平台目录不完整 → FAIL。
  - **结构反模式检查（2026-08-07 起生效）**：公众号存在独立「我的系统/自我实证」章节、结尾仅复述前文数据、行业话题未回答「为什么火/钱归谁」→ 退回重写。
  - **数据可视化硬门（C11-C12）**：公众号 HTML 必须含 ≥2 个 `data-viz` 组件且无 `[[IMG:` 残留；小红书 slides 必须含条形/占比可视化。
  - **评分报告落盘**：跑完 `harsh_critic_score.py` 后必须执行 `python3 scripts/generate_score_report.py outputs/<job_id>/` 生成机器初筛版 `评分报告.md`，再由人工逐条复核 Hook 六维/事实来源/视觉排版后定稿。
  - **公众号预览必交付**：公众号排版 HTML 完成后必须执行 `python3 scripts/generate_gzh_preview.py --input outputs/<job_id>/公众号/gzh_<主题>_排版_<主题色>.html --screenshot outputs/<job_id>/公众号/mobile_check.png`，生成 `_预览.html`（一键复制到公众号）与移动端截图。
  - **状态机强制**：`run_daily_pipeline.py --qa outputs/<job_id>/` 缺 `state.json` 或状态未到 draft/visual 时直接 REJECTED；机器 PASS 后自动推进 `review → archive`。
...
- **发布就绪（人工终审模式）**：呈现最终 Checklist 与成果文件卡片，等待用户回复“确认发布”：
  - **公众号草稿（官方 API，推荐）**：`python3 scripts/gzh_draft_api.py --title ... --content-file ... --cover ... --job-id <job_id>` 调用微信 `draft/add` 接口，存入公众号草稿箱，人工手机终审。
  - **小红书（人工上传，风控合规）**：直接交付 `outputs/<job_id>/小红书/`（xhs-01~05.png + 文案.md，即成品文件夹本身，不另建发布素材包），由用户在手机/网页端手动上传发布；发布后 `python3 scripts/record_manual_publish.py <job_id> --platform 小红书` 标记记录。
  - **风控铁律**：小红书禁止任何自动化工具写入/发布（Playwright、xhs_publisher、n8n 小红书工作流均已下线），发布动作必须由人手完成。
  - 公众号草稿与小红书手动标记均落盘 `jobs/<job_id>/publish_log.json`。
...
1. **【决策卡点 1：选题决策】**
   - 采编 Agent 在对话框推荐 3-5 个热点选题（含谷歌趋势/X 热点海外源时，必须先做「国内可合规发布」复核）。
   - **用户操作**：必须由用户（老板）回复选题编号后才建 Job 进入创作；**超时未回复不自动选题**，停在 `topic` 等待（除非用户明确要求 `--auto-select`）。
2. **【决策卡点 2：草稿箱同步确认】**
   - 全套图文、卡片与排版生成完毕且完成临时文件清扫后，归档发布员呈现预览 Checklist。
   - **用户操作**：回复 `确认发布`，系统将公众号排版存入官方草稿箱、小红书产出文件夹就绪（`outputs/<job_id>/小红书/`）；公众号由用户在手机端终审发布，小红书由用户手动上传发布。
