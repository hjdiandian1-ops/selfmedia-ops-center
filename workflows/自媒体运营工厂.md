...
- **发布就绪（人工终审模式）**：呈现最终 Checklist 与成果文件卡片，等待用户回复“确认发布”：
  ...
  - **小红书（人工上传，风控合规）**：`python3 scripts/prepare_xhs_material.py <job_id>` 生成 `outputs/<job_id>/小红书发布素材包/`（图片+文案+发布说明），由用户在手机/网页端手动上传发布；发布后 `python3 scripts/record_manual_publish.py <job_id> --platform 小红书` 标记记录。
...
2. **【决策卡点 2：草稿箱同步确认】**
   ...
   - **用户操作**：回复 `确认发布`，系统将公众号排版存入官方草稿箱、生成小红书发布素材包；公众号由用户在手机端终审发布，小红书由用户手动上传发布。
