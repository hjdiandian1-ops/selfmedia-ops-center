...
- **公众号主编 (WeChat Longform Chief Editor)**：专职公众号结构化深度长文创作，注入极客操盘手观点。必须调用 `/dbs-hook` 诊断开头；真实感硬指标（≥2 具体数字 + ≥1 真实项目名 + ≥1 第一人称经历，无种子素材时向用户索取，严禁脑补）。**正文必须含 ≥2 个 `data-viz` 组件（表格/条形/占比/KPI），复杂对比用 PNG 图卡；统一调用 `scripts/generate_data_viz.py`（C11 校验），数据只取素材包真实数据/用户投喂。**
