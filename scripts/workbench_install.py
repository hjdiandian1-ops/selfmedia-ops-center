...
def main():
    ...
    if sys.version_info < (3, 11):
        print(f"❌ Python 版本过低：{sys.version.split()[0]}，需要 3.11 以上")
        return 1
    ...
    subprocess.run([venv_python, "-m", "pip", "install", "-q", "--upgrade", "pip"], check=False)  # nosec B603  # 固定命令
    subprocess.run(  # nosec B603  # 固定命令
        [venv_python, "-m", "pip", "install", "-r", os.path.join(ROOT, "requirements.lock")],
        check=True,
    )
