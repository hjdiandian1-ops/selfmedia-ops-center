...
async def publish_note(payload: PublishPayload):
    ...
    raw_images = payload.images if isinstance(payload.images, list) else [payload.images]
    valid_images = []
    ...
    if isinstance(payload.tags, str):
        raw_tags = [t.strip() for t in payload.tags.split(',') if t.strip()]
    elif isinstance(payload.tags, list):
