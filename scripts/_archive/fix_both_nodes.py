...
write_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_q} | base64 -d > /volume1/docker/n8n/postgres_data/get_wf6.sql'"
stdin, stdout, stderr = ssh.exec_command(write_cmd)
stdout.read()

run_cmd = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -t -f /var/lib/postgresql/data/get_wf6.sql"
...
for n in nodes:
    if n.get("name") == "分支 1：小红书自动发布 (Playwright)":
        ...
        n["parameters"]["url"] = "http://xhs-publisher:8000/publish"
    elif n.get("name") == "分支 2：公众号自动发布/草稿":
        n["continueOnFail"] = True
        n["parameters"]["url"] = "http://creator_backend:3000/api/gzh/publish"
...
write_sql_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_sql} | base64 -d > /volume1/docker/n8n/postgres_data/fix_both_nodes.sql'"
stdin, stdout, stderr = ssh.exec_command(write_sql_cmd)
stdout.read()

run_update = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -f /var/lib/postgresql/data/fix_both_nodes.sql"
stdin, stdout, stderr = ssh.exec_command(run_update)
print("Update Both Nodes Out:\n", stdout.read().decode())
