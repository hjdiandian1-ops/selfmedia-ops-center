...
write_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_q} | base64 -d > /volume1/docker/n8n/postgres_data/get_wf12.sql'"
stdin, stdout, stderr = ssh.exec_command(write_cmd)
stdout.read()

run_cmd = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -t -f /var/lib/postgresql/data/get_wf12.sql"
...
for n in nodes:
    if n.get("name") == "分支 1：小红书自动发布 (Playwright)":
        n["parameters"] = {
            ...
            "sendBody": True,
            "bodyType": "json",
            "jsonBody": '="{\\"title\\":" + JSON.stringify($json.title) + ",\\"content\\":" + JSON.stringify($json.xhs_content) + ",\\"images\\":" + JSON.stringify($json.images) + ",\\"tags\\":" + JSON.stringify($json.tags) + ",\\"cookies_json_path\\":\\"/data/shared/xhs_cookies.json\\"}"',
            "options": {}
        }
        ...
        n["parameters"] = {
            ...
            "sendBody": True,
            "bodyType": "json",
            "jsonBody": '="{\\"title\\":" + JSON.stringify($json.title) + ",\\"content_html\\":" + JSON.stringify($json.gzh_html) + "}"',
            "options": {}
        }
        n["continueOnFail"] = True
...
SET nodes = '{nodes_json_str.replace("'", "''")}'::json
WHERE "workflowId" = 'Qald4KugzwLNNxQq';
"""

b64_sql = base64.b64encode(sql_update.encode("utf-8")).decode("utf-8")
write_sql_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_sql} | base64 -d > /volume1/docker/n8n/postgres_data/update_both_tables.sql'"
stdin, stdout, stderr = ssh.exec_command(write_sql_cmd)
stdout.read()

run_update = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -f /var/lib/postgresql/data/update_both_tables.sql"
stdin, stdout, stderr = ssh.exec_command(run_update)
print("Update Both Tables Out:\n", stdout.read().decode())
