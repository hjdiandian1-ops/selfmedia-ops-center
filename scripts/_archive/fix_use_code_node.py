...
write_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_q} | base64 -d > /volume1/docker/n8n/postgres_data/get_final2.sql'"
stdin, stdout, stderr = ssh.exec_command(write_cmd)
stdout.read()

run_cmd = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -t -f /var/lib/postgresql/data/get_final2.sql"
stdin, stdout, stderr = ssh.exec_command(run_cmd)
raw_wf = stdout.read().decode().strip()
wf = json.loads(raw_wf)
nodes = wf.get("nodes", [])

# Replace XHS HTTP Request node with Code node that uses $http
# n8n Code node supports $http.request() in workflow code
xhs_code = """
const payload = {
  title: $input.first().json.title,
  content: $input.first().json.xhs_content,
...
for n in nodes:
    if n.get("name") == "分支 1：小红书自动发布 (Playwright)":
        ...
        print(f"✅ Changed XHS node to Code node")
...
write_sql_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_sql} | base64 -d > /volume1/docker/n8n/postgres_data/fix_code_node.sql'"
stdin, stdout, stderr = ssh.exec_command(write_sql_cmd)
stdout.read()

run_update = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -f /var/lib/postgresql/data/fix_code_node.sql"
