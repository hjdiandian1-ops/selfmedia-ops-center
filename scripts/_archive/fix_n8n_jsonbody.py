...
write_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_q} | base64 -d > /volume1/docker/n8n/postgres_data/get_wf3.sql'"
stdin, stdout, stderr = ssh.exec_command(write_cmd)
stdout.read()

run_cmd = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -t -f /var/lib/postgresql/data/get_wf3.sql"
...
for n in nodes:
    if n.get("name") == "分支 1：小红书自动发布 (Playwright)":
        n["parameters"]["jsonBody"] = "={{ JSON.stringify({ title: $json.title, content: $json.xhs_content, images: $json.images, tags: $json.tags, cookies_json_path: \"/data/shared/xhs_cookies.json\" }) }}"
...
write_sql_cmd = f"echo {NAS_PASS} | sudo -S bash -c 'echo {b64_sql} | base64 -d > /volume1/docker/n8n/postgres_data/update_jsonbody.sql'"
stdin, stdout, stderr = ssh.exec_command(write_sql_cmd)
stdout.read()

run_update = f"echo {NAS_PASS} | sudo -S {DOCKER_BIN} exec n8n_postgres psql -U n8n -d n8n -f /var/lib/postgresql/data/update_jsonbody.sql"
